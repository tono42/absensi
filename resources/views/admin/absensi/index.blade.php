<x-app-layout>
    ...
    <div class="py-10" x-data="absensiPage">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md flex items-center gap-4">
                    <div class="bg-blue-100 dark:bg-blue-900/40 p-3 rounded-full"><i class="fas fa-list-ol text-2xl text-blue-500"></i></div>
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Total Data</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $absensi->total() }}</p>
                    </div>
                </div>
                <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md flex items-center gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Sudah Divalidasi</p>
            ...
            <section class="bg-white dark:bg-gray-800 shadow-lg rounded-xl overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">Laporan & Validasi Absensi</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Filter, cari, dan validasi data kehadiran karyawan.</p>
                    
                    {{-- Filter Form --}}
                    <form method="GET" action="{{ route('admin.absensi.index') }}" class="mt-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                            ...
                            <input type="date" name="tanggal" value="{{ request('tanggal') }}" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-white focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div class="flex space-x-2">
                                ...
                                <a href="{{ route('admin.absensi.index') }}" class="w-full flex items-center justify-center px-4 py-2 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 font-semibold rounded-lg transition" title="Reset Filter">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left font-semibold text-gray-600 dark:text-gray-300 uppercase">Check In / Out</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($absensi as $item)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">{{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('d M Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">
                                        <span class="font-semibold text-green-600">{{ $item->check_in ? \Carbon\Carbon::parse($item->check_in)->format('H:i') : '-' }}</span> / 
                                        <span class="font-semibold text-red-600">{{ $item->check_out ? \Carbon\Carbon::parse($item->check_out)->format('H:i') : '-' }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        @if($item->status == 'pending')
                                            <div class="flex justify-center items-center gap-2">
                                                <button @click="openModal('{{ route('admin.absensi.validate', $item) }}', 'approved', '{{ $item->user->name }}')" class="px-3 py-1 text-xs bg-green-500 text-white rounded-md hover:bg-green-600 transition" title="Approve">
                                                    <i class="fas fa-check"></i> Approve
                                                </button>
                                                <button @click="openModal('{{ route('admin.absensi.validate', $item) }}', 'rejected', '{{ $item->user->name }}')" class="px-3 py-1 text-xs bg-red-500 text-white rounded-md hover:bg-red-600 transition" title="Reject">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <p>Tidak ada data absensi yang cocok dengan filter Anda.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <!-- Validation Modal -->
        <div x-show="showModal" @keydown.escape.window="closeModal" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;">
                        ...
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">Konfirmasi Validasi</h3>
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            Anda akan <b :class="formStatus === 'approved' ? 'text-green-600' : 'text-red-600'" x-text="formStatus === 'approved' ? 'menyetujui' : 'menolak'"></b> absensi untuk karyawan <b class="text-gray-800 dark:text-gray-200" x-text="karyawanName"></b>. Apakah Anda yakin?
                                        </p>
                                    </div>
                                </div>
                            </div>
